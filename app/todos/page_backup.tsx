        ...
        <div className="max-w-7xl mx-auto px-6 py-5">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">Welcome, {username || 'hninnn'}</p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
          <form onSubmit={createTodo} className="space-y-4">
            ...
            <div className="flex items-center gap-4 text-sm text-gray-700">
              ...
              <label className="text-gray-600">Reminder:</label>
              <select
                value={reminder ?? ''}
                onChange={(e) => {
            ...
            <div className="flex items-center gap-3 text-sm">
              <label className="text-gray-700 whitespace-nowrap">Use Template:</label>
              <select
                value={template}
                onChange={(e) => setTemplate(e.target.value)}
                className="flex-1 px-3 py-2 bg-white border border-gray-300 rounded-lg text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
                  ...
                  {tags.map(tag => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-full text-sm font-medium transition-all ${selectedTagIds.includes(tag.id)
                        ...
                        : 'border-2 text-gray-700 hover:text-gray-900 bg-white'
                        }`}
                      style={{
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200 space-y-4">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-gray-100 text-gray-700 rounded-lg text-sm hover:bg-gray-200 transition-colors font-medium"
          >
        ...
        {sortedAndFilteredTodos.length === 0 ? (
          ...
          <div className="space-y-4">
            <h2 className="text-xl font-bold text-blue-600">Pending ({sortedAndFilteredTodos.filter(t => !t.completed).length})</h2>
                  ...
                  {editingId === todo.id ? (
                          ...
                          <div className="flex flex-wrap gap-2">
                            {tags.map(tag => (
                              <button
                                ...
                                className={`px-3 py-1.5 rounded-full text-sm font-medium transition-all ${editSelectedTagIds.includes(tag.id)
                                  ? 'text-white'
                    ...
                    <div className="flex items-start gap-4">
                      <input
                        ...
                        className="mt-1 w-5 h-5 cursor-pointer accent-blue-600 rounded border-gray-300"
                      />
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-2">
                          <p className={`text-base font-medium ${todo.completed ? 'line-through text-gray-400' : 'text-gray-900'}`}>
                            {todo.title}
                          </p>
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded text-xs font-semibold ${todo.completed ? 'opacity-50' : ''
                            } ${
                              todo.priority === 'high' ? 'bg-yellow-100 text-yellow-800 border border-yellow-200' :
                          ...
                          {todo.tags && todo.tags.length > 0 && (
                            <>
                              {todo.tags.map(tag => (
                                <span
                                  key={tag.id}
                                  className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium text-white ${todo.completed ? 'opacity-50' : ''
                                    }`}
                        ...
                        {todo.due_date && (
                          <p className={`text-sm ${
                            ...
                            {isPastDue(todo.due_date) && !todo.completed ? 'Due in 4 hours ' : '📅 '}{formatSingaporeDate(todo.due_date)}
                          </p>
                        )}
                      ...
                      <div className="flex gap-2 flex-shrink-0">
                        <button
                          ...
                          className="px-3 py-1.5 text-gray-700 hover:bg-gray-100 rounded-lg text-sm font-medium transition-colors flex items-center gap-1"
                        >
                          ...
                          {todo.tags && todo.tags.length > 0 && (
                            <>
                              {todo.tags.map(tag => (
                                <span
                                  key={tag.id}
                                  className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium text-white ${todo.completed ? 'opacity-50' : ''
                                    }`}
                                  style={{ backgroundColor: tag.color }}
                                >
                                  {tag.name}
            ...
            <div className="flex flex-wrap gap-2">
              {uniqueCategories.map(cat => (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${selectedCategoryFilter === cat
                ...
                >
                  {editingTemplateId === template.id ? (
                    ...
                    <>
                      ...
                      {/* Priority Badge */}
                      <span className={`inline-block px-2 py-1 text-xs font-semibold rounded mb-2 ${template.priority === 'high' ? 'bg-red-500/20 text-red-400' :
